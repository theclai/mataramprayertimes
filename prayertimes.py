...
class PrayTimes():
	...
	methods = {
		'MWL': {
			...
			'params': { 'subuh': 18, 'isha': 17 } },
		'ISNA': {
			'name': 'Islamic Society of North America (ISNA)',
			'params': { 'subuh': 15, 'isha': 15 } },
		'Egypt': {
			'name': 'Egyptian General Authority of Survey',
			'params': { 'subuh': 19.5, 'isha': 17.5 } },
		'Makkah': {
			'name': 'Umm Al-Qura University, Makkah',
			'params': { 'subuh': 18.5, 'isha': '90 min' } },  # subuh was 19 degrees before 1430 hijri
		'Karachi': {
			'name': 'University of Islamic Sciences, Karachi',
			'params': { 'subuh': 18, 'isha': 18 } },
		'Tehran': {
			'name': 'Institute of Geophysics, University of Tehran',
			'params': { 'subuh': 17.7, 'isha': 14, 'maghrib': 4.5, 'midnight': 'Jafari' } },  # isha is not explicitly specified in this method
		'Jafari': {
			'name': 'Shia Ithna-Ashari, Leva Institute, Qum',
			'params': { 'subuh': 16, 'isha': 14, 'maghrib': 4, 'midnight': 'Jafari' } }
	}
	...
	# compute ashar time
	def asharTime(self, factor, time):
		decl = self.sunPosition(self.jDate + time)[0]
	...
	def computePrayerTimes(self, times):
		...
		subuh    = self.sunAngleTime(self.eval(params['subuh']), times['subuh'], 'ccw')
		sunrise = self.sunAngleTime(self.riseSetAngle(self.elv), times['sunrise'], 'ccw')
		dhuhur   = self.midDay(times['dhuhur'])
		ashar     = self.asharTime(self.asharFactor(params['ashar']), times['ashar'])
		...
		return {
			...
			'ashar': ashar, 'sunset': sunset, 'maghrib': maghrib, 'isha': isha
		}
	...
	def computeTimes(self):
		times = {
			...
			'ashar': 13, 'sunset': 18, 'maghrib': 18, 'isha': 18
		}
		# main iterations
		...
		if self.settings['midnight'] == 'Jafari':
			times['midnight'] = times['sunset'] + self.timeDiff(times['sunset'], times['subuh']) / 2
		else:
	...
	def adjustTimes(self, times):
		...
		if self.isMin(params['imsak']):
			times['imsak'] = times['subuh'] - self.eval(params['imsak']) / 60.0
		...
		times['dhuhur'] += self.eval(params['dhuhur']) / 60.0

		return times

	# get ashar shadow factor
	def asharFactor(self, asharParam):
		methods = {'Standard': 1, 'Hanafi': 2}
		return methods[asharParam] if asharParam in methods else self.eval(asharParam)
	...
	def adjustHighLats(self, times):
		...
		times['subuh']  = self.adjustHLTime(times['subuh'], times['sunrise'], self.eval(params['subuh']), nightTime, 'ccw')
	...
	def adjustHLTime(self, time, base, angle, night, direction = None):
		...
		if self.isNan(time) or diff > portion:
